...
test('Update Product', async ({ request }) => {
    ...
    const response = await request.put(`/products/${productId}`, {
        ...
        data: {
            title: process.env.TITLE_PRODUCT_UPDATE,
            description: process.env.DESCRIPTION_PRODUCT_UPDATE,
            price: Number(process.env.PRICE_PRODUCT_UPDATE),
        },
    });

    const responseBody = await response.json();
    expect(response.status()).toBe(200);
    expect(responseBody.title).toBe(process.env.TITLE_PRODUCT_UPDATE);
    expect(responseBody.price).toBe(Number(process.env.PRICE_PRODUCT_UPDATE));
    expect(responseBody.description).toBe(process.env.DESCRIPTION_PRODUCT_UPDATE);
